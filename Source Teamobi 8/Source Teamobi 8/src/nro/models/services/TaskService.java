...
import nro.models.task.SideTask;
...
public class TaskService {
    ...
    public void paySideTask(Player player) {
        if (player == null || player.playerTask == null || player.playerTask.sideTask == null
            ...
            Service.gI().sendThongBao(player, "Không có nhiệm vụ để trả!");
            return;
        }
        ...
        int goldReward = 0;     // vàng thường (tiền)
        int goldBarQty = 0;     // số thỏi vàng (item 457) sẽ thưởng
        ...
        // cần tối thiểu 1 ô trống
        if (InventoryService.gI().getCountEmptyBag(player) < 1) {
            Service.gI().sendThongBao(player, "Hành trang không đủ chỗ trống.");
            return;
        }

        // tạo thỏi vàng đúng số lượng
        Item thoiVang = ItemService.gI().createNewItem((short) 457, goldBarQty);

        // add vào túi & gửi cập nhật
        ...
        Service.gI().sendThongBao(
            player,
            "Bạn nhận được " + goldBarQty + " thỏi vàng"
            + " 🎉"
        );

        // reset nhiệm vụ sau khi trả
        st.reset();
    }
