...
public class PlayerDAO {
    ...
    ; 
       // acctive
    public static boolean updateActivated(Player p, boolean value) {
        try {
            int affected = LocalManager.executeUpdate(
    ...
    public static boolean addGoldBar(Player player, int num) {
        if (num <= 0) return false;
        String sql = "UPDATE account SET thoi_vang = thoi_vang + ?, active = ? WHERE id = ?";
        try (Connection con = LocalManager.getConnection();
            ...
            ps.executeUpdate();
            player.getSession().goldBar += num; // đồng bộ session
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
   ...
   // ĐỌC JSON từ DB
        public static Map<Byte, Long> loadDailyGiftsJson(Player p) {
            Map<Byte, Long> map = new HashMap<>();
            final String sql = "SELECT daily_gifts_json FROM account WHERE id = ?";

            try (Connection con = LocalManager.getConnection();
                PreparedStatement ps = con.prepareStatement(sql)) {

                ps.setLong(1, p.getSession().userId);       
                    ...
                    if (rs.next()) {
                        ...
                        if (obj != null) {
                            for (Map.Entry<String, JsonElement> e : obj.entrySet()) {
                                try {
                                    map.put(Byte.parseByte(e.getKey()), e.getValue().getAsLong());
        ...
        // GHI JSON xuống DB
        public static boolean saveDailyGiftsJson(Player p, Map<Byte, Long> map) {
            JsonObject obj = new JsonObject();
            for (Map.Entry<Byte, Long> e : map.entrySet()) {
                obj.addProperty(Byte.toString(e.getKey()), e.getValue());
            }
            String json = obj.toString();

            String sql = "UPDATE account SET daily_gifts_json = ? WHERE id = ?"; 
            try (Connection con = LocalManager.getConnection();             
                PreparedStatement ps = con.prepareStatement(sql)) {
                ps.setString(1, json);
                ps.setLong(2, p.getSession().userId);
                return ps.executeUpdate() > 0;
            } catch (Exception ex) { ex.printStackTrace(); return false; }
        }
        //SAVE VND điểm danh hàng ngày
        public static boolean saveVnd(Player player) {
            if (player == null || player.getSession() == null) return false;
            String sql = "UPDATE account SET vnd = ? WHERE id = ?";
            try (Connection con = LocalManager.getConnection();
                PreparedStatement ps = con.prepareStatement(sql)) {
                ps.setLong(1, player.getSession().vnd);
